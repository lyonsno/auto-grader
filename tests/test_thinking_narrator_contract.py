...
class _DossierWithLegibilityAfterActionNarrator(_DossierAfterActionNarrator):
    def _handle_legibility_rows(self, prediction, item):  # type: ignore[override]
        return ThinkingNarrator._handle_legibility_rows(self, prediction, item)
...
class ThinkingNarratorContract(unittest.TestCase):
    ...
    def test_background_dossier_prompt_carries_rubric_and_acceptable_band_context(self):
        sink = _DummySink()
        narrator = _DossierWithLegibilityAfterActionNarrator(sink)
        item = type(
            "Item",
            (),
            {
