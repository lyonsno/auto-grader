...
def render_focus_preview(page_png: bytes, focus_region: FocusRegion) -> bytes:
    ...
    The crop is feathered toward the terminal-matched background
    within the crop rectangle itself, rounded at the corners, and
    gets a soft accent outline ring just inside the content edge.
    Edge-effect sizes are proportional to the smaller image
    dimension so the visual result is consistent across source DPIs.
    """
